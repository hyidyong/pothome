...
- 로컬 Supabase reset, pgTAP 61/61, database lint, security/performance advisor를 검증했다. Auth·Realtime·Storage·Studio·Analytics는 비활성화했고 DB 네트워크는 loopback CIDR만 허용한다. 앱 개발 서버는 `127.0.0.1`에만 바인딩한다. 현재 Windows Docker Desktop은 API 포트를 `0.0.0.0`에 게시하며 관리자 권한이 없어 프로젝트 전용 Firewall 규칙을 만들지 않았다. 따라서 Windows Firewall을 켜고 사용하며, API는 RLS 공개 읽기 전용 데이터만 노출한다. custom Docker network는 DB reset 뒤 내부 DNS를 분리하므로 사용하지 않는다. 호스팅 프로젝트 생성·연결과 Vercel 배포는 사용자 승인 전까지 보류한다.
