...
- 동적 Decision Spine: 1008px scrub 범위에서 단계 01 → 02 → 03 → 04가 활성화되고 indicator scale이 0.25 → 0.34 → 0.67 → 1로 진행했다. intro에는 transform을 적용하지 않았다.
...
- Hero는 수정하지 않은 1672×941 원본에서 AVIF 64,206B와 WebP 82,712B를 만들었다. Next Image가 AVIF를 전송했으며 첫 로컬 측정은 transfer 13,970B, 약 131ms였다.
...
- 사용자가 확정한 민감자료 선별 결정을 반영했다. 계약·동의서·합류제안서, 개인 대화, 혼합 강의자료·복제물, 중복본과 `~$` 파일은 내용을 열지 않고 경로만 EXCLUDE 처리했다. 직접 작성한 과제·포트폴리오·화면 로직 등은 읽기 허용일 뿐 공개 승인으로 간주하지 않는다.
